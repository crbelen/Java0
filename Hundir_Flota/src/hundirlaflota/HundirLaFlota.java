...
public class HundirLaFlota {
	...
	public static void main(String[] args) {
		// La clase Scanner nos permite, junto con System.in, leer desde teclado
		...
		while (!juegoTerminado) {
			...
			while (!tiroCorrecto) {
				...
				if (tiro[0] != -1 && tiro[1] != -1) {
					...
					System.out.println("TIRO CORRECTO");
				}
			...
			char tipoTiro = (puntosOrdenadorAnterior - puntosOrdenador)> 0 ? TOCADO: AGUA; //op ternario
			...
			//Si no ha ganado el jugador, le toca a la maquina
			if (!juegoTerminado) {
	...
	/*
	 * Metodo que aglutina la inicializacion de ambos mapas
	 */
	...
	/* 
	 * Metodo que inicializa el mapa que mostramos al usuario
	...
	/*
	 * Metodo que inicializa un mapa de juego, 
	 * colocando los barcos sobre el mismo
	 */
	private static void inicializaMapa(char[][] mapa) {
		...
		for (int b : barcos) {
			...
			//Intentamos tantas veces sea necesarias para colocar el barcon en el mapa.
			//Vamos de mayor tamaño a menor, para que sea menos dificil encontrar un hueco.
			...
			while (!colocado) {
				...
				//el resto de dividir entre 2 o bien nos dara 0 o bien 1, obteniendo las dos direcciones
				
				//¿Cabe el barco en la posicion indicada?
				if (direcc == 'V') {
					if (fila + b <= (TAMANIO - 1)) {
	...
	/*
	 * Metodo mediante el cual el usuario introduce una casilla 
	 */
	private static int[] pedirCasilla() {
		...
		if (linea.matches("^[A-Z][0-9]*$")) {
			...
			//Para la columna, tan solo tenemos que procesar el numero
			int columna = Integer.parseInt(linea.substring(1,linea.length()));
			//Si las coordenadas estan dentro del tamaño del tablero, las devolvemos
	...
	/*
	 * Metodo que nos permite evaluar si un tiro es CORRECTO (AGUA O TOCADO)
	 * o se trata de una casilla por la uqe ya hemos pasado antes.
	 */
	private static boolean evaluarTiro(char[][] mapa, int[] t) {
		int fila = t[0];
		int columna = t[1];
	...
	/*
	 * Metodo que actualiza el mapa, con un determinado tiro.
	 * Devolvemos el numero de puntos restantes
	 */
	...
	/*
	 * Metodo que sirve para que el ordenador pueda hacer un disparo
	 */
